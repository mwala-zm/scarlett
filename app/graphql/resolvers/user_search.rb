module Resolvers
  class UserSearch < Resolvers::BaseSearchResolver
    ...
    option(:reset_password_sent_at, type: GraphQL::Types::ISO8601DateTime) do |scope, value|
      scope.where reset_password_sent_at: value
    end
    option(:allow_password_change, type: Boolean) { |scope, value| scope.where allow_password_change: value }
    option(:remember_created_at, type: GraphQL::Types::ISO8601DateTime) do |scope, value|
      scope.where remember_created_at: value
    end
    ...
    option(:confirmation_sent_at, type: GraphQL::Types::ISO8601DateTime) do |scope, value|
      scope.where confirmation_sent_at: value
    end
    option(:unconfirmed_email, type: String) { |scope, value| scope.where unconfirmed_email: value }
    ...
    option(:current_sign_in_at, type: GraphQL::Types::ISO8601DateTime) do |scope, value|
      scope.where current_sign_in_at: value
    end
    option(:last_sign_in_at, type: GraphQL::Types::ISO8601DateTime) do |scope, value|
      scope.where last_sign_in_at: value
    end
    ...
    option(:first_name, type: String) { |scope, value| scope.where first_name: value }
    option(:last_name, type: String) { |scope, value| scope.where last_name: value }
    option(:middle_name, type: String) { |scope, value| scope.where middle_name: value }
  end
end
